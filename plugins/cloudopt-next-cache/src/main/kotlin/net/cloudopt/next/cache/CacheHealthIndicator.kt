        ...
        return await {
            ...
            CacheManager.regions.forEach { (name, cache) ->
                result.data[name] = mutableMapOf(
                    ...
                    "loadFailureCount" to cache.stats().loadFailureCount(),
                    "totalLoadTime" to cache.stats().totalLoadTime(),
                    "evictionCount" to cache.stats().evictionCount(),
                    "evictionWeight" to cache.stats().evictionWeight()
                )
            }
            return@await result
        }
